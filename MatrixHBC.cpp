...
void MatrixHBC::CalculateMatrixHBC(Element element)
{
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			matrixH[i][j] = element.isSurfaceHeated[0] * sum1[i][j] + element.isSurfaceHeated[1] * sum2[i][j] + element.
				isSurfaceHeated[2] * sum3[i][j] + element.isSurfaceHeated[3] * sum4[i][j];
